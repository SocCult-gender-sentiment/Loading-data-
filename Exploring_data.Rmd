...
#making a df for each faculty just to investigte which Research areas are in which faculty
...
#The different faculty columns are combined into one. 
...
#Clean the Faculty column - that is messy after using the unite function
...
df_test <- mutate(df_test, Facultyfinal = ifelse(Faculty == "",
                                            'h', ''))
#combining the two faculty columns 
df_test <- df_test %>%  unite("Faculty", Faculty:Facultyfinal, remove = FALSE)

#Cleaning the Faculty column after the unite function 
df_test <- df_test %>%
  mutate(
    Faculty = str_replace_all(Faculty, "_", "")
   )
...
df <- read_csv("df_w_faculty.csv") #remove later 
...
###Trying to make the lix thing work - but can't... cry
