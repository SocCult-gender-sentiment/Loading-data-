...
dflix <- dflix %>% select(wordcount, n_breaks, long_word, LIX)
...
df_gender <- dfsen %>%  filter(First.Author.Gender.Probability < 0.70 & Last.Author.Gender.Probability < 0.70)

#removing them from our data 

dfsen <- dfsen %>% filter(!(First.Author.Gender.Probability < 0.70 & Last.Author.Gender.Probability < 0.70))

dfsen<- dfsen %>% mutate(First.Author.Gender = ifelse(First.Author.Gender.Probability < 0.70, NA, First.Author.Gender))
dfsen<- dfsen %>% mutate(Last.Author.Gender = ifelse(Last.Author.Gender.Probability < 0.70, NA, Last.Author.Gender))
